...
export const getFarmersForProduct = async (req, res) => {
  try {
    ...
    if (!consumerPincode) {
      return res.status(400).json({
        success: false,
        message: "Consumer pincode is required",
      });
    }
    ...
    if (!consumerCoords) {
      return res.status(400).json({
        success: false,
        message: "Unable to retrieve coordinates for consumer pincode",
      });
    }
    ...
    if (farmers.length === 0) {
      return res.status(404).json({
        success: false,
        message: `No farmers found within ${maxDistance} km selling this product`,
      });
    }
